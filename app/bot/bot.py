    ...
    if request.data:
        ...
        if request_type == 'message_new':
            ...
            if "начать" in text:
                ...
                reply(peer_id=peer_id, attachment="photo-201379365_457239017")
            elif "расписание на сегодня" in text:
                reply(peer_id=peer_id, message=raspisanie[get_today()][get_week()])
            elif "расписание на завтра" in text:
                reply(peer_id=peer_id, message=raspisanie[get_today() + 1][get_week(True)])
            elif "расписание пар" in text:
                reply(peer_id=peer_id, message=raspisanie_par)
            elif "фио преподавателей" in text:
                reply(peer_id=peer_id, message=prepody)
